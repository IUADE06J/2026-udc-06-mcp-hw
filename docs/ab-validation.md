...
## Прогін B — без catalog

**Як саме вимикали:** вимкнено лише сервер `catalog`; `filesystem` і `memory` лишались підключеними. Далі **новий чат** (не продовження сесії A). Старий чат з живими catalog-tools не використовували. Після запису `catalog` знову потрібен у `.cursor/mcp.json` для Task B.

**Що зробив агент:** не відмовився і не видав правдоподібну вигадку. Відповідь називає правило `stock <= reorderLevel` і `sum(price × stock)` по 24 позиціях seeded-каталогу й не посилається на `low_stock` / `inventory://catalog`. Лог викликів того чату сюди не копіювали, лише фінальний текст — **джерело даних невідоме** (не стверджуємо `read_text_file` чи читання `app/data/catalog.json`).
...
|---|---|---|
| Викликав tool | так: `low_stock` + resource `inventory://catalog` | catalog-tools не згадуються у відповіді; логу викликів немає, джерело даних невідоме |
...
MCP **не змінив правильність**: A (з `catalog`) і B (новий чат без `catalog`, filesystem і memory лишались) обидва дали 9 SKU і **46152**. Це чесний нульовий результат.

Різниця в шляху для A відома: правило з протестованого `app/` через `low_stock` і `inventory://catalog`. Для B відомо лише фінальний текст — як саме агент дістав числа, лог не фіксує. Висновок: вимкнення `catalog` не зламало відповідь, поки інші джерела в хості лишаються. Власний сервер вартий як стабільний API над `app/`, не як доказ, що без нього агент «не може» порахувати каталог.
